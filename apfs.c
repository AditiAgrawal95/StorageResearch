...
void handle_drec(char *dirName, j_drec_val_t dir)
{
	...
	if ((dir.flags & DREC_TYPE_MASK) == DT_DIR) {
		...
		if (dir.file_id == ROOT_DIR_INO_NUM)
			;//printf(ANSI_COLOR_RED "%s:" ANSI_COLOR_RESET, ToUp(dirName));
		else
...
btree_node_phys_t parseAPFSVolumeBlock(FILE *apfs, apfs_superblock_t volumeSuperBlock,APFS_SuperBlk containerSuperBlk,int *endOfOmapBtreeBtree, command_line_args args)
{
	// Go to omap id
...
void parse_APFS(command_line_args args, char* filename)
{
	FILE *apfs = NULL;
	APFS_SuperBlk containerSuperBlk;
	omap_phys_t omapStructure;
	...
	fsTree=parseAPFSVolumeBlock(apfs,volumeSuperBlock,containerSuperBlk,&endOfOmapBtreeBtree, args);
	parseFSTree(apfs,fsTree,endOfOmapBtreeBtree, args);
}
